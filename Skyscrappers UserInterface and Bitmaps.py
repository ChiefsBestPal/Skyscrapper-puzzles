...
out = Image.new(
    "RGB", (cell_width * Board_Size, cell_height * Board_Size), (255, 255, 255)
)
width, height = out.size[0], out.size[1]
AntEnderfnt = ImageFont.truetype(
    "C:/Users/Antoine/Desktop/Coding+/Animation/AntenderHandwriting-Regular-Final1.ttf",
    40,
)
...
x_dephasage_manuel_estimation_text = (
    deph_x
) = (
    -13
)  # x = 37 centers letter X with vertical vertices instead of margin of 50 (default)
...
for c, i in enumerate(range(1, Board_Size + 1)):
    ...
    sep1, sep2 = [
        (p[0], p[1] + (new_cell_height * i)) for p in [p1, p2]
    ]  # sep horizontales
    sep3, sep4 = [
        (p[0] + (new_cell_width * i), p[1]) for p in [p1, p3]
    ]  # sep verticales
    ...
    clues_positions.append(
        (deph_x + p1[0] + ((new_cell_width // 2 * mt)), p1[1] - second_margin)
    )
    ...
    clues_positions.append(
        (
            p2[0]
            + second_margin
            // 2,  # second_margin//2 because max pixel is weird with letters and wont show unlike the 0th one
            deph_y + p1[1] + ((new_cell_height // 2 * mt)),
        )
    )

    clues_positions.append(
        (deph_x + p3[0] + ((new_cell_width // 2 * mt)), p3[1] + second_margin // 2 - 20)
    )  # second_margin//2 -20 becau1se max pixel is weird with letters and height of letter is often bigger than width

    clues_positions.append(
        (p1[0] - second_margin, deph_y + p1[1] + ((new_cell_height // 2 * mt)))
    )
print((clues_positions))

sorted_clue_coords = sorted(
    clues_positions, key=lambda x: clues_positions.index(x) % 4, reverse=False
)
...
final_coords_clues = (
    chunk_clue_coords[0]
    + chunk_clue_coords[1]
    + chunk_clue_coords[2][::-1]
    + chunk_clue_coords[3][::-1]
)

for ix_clue, clue_position in enumerate(
    final_coords_clues
):  # !put in class for object as input
